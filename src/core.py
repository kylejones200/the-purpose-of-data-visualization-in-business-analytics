...
def generate_sales_data(months: int=12, regions: list | None=None, seed: int=42) -> pd.DataFrame:
    """Generate synthetic sales data."""
    if regions is None:
        regions = ['North', 'South', 'West']
    np.random.seed(seed)
    date_range = pd.date_range('2022-01-01', periods=months, freq='M')
    data = {'Month': np.tile(date_range, len(regions)), 'Region': np.repeat(regions, len(date_range)), 'Sales': np.random.randint(90, 150, size=len(date_range) * len(regions))}
